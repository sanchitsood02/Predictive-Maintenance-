#!/usr/bin/env python3
"""
N-CMAPSS Statistical Analysis - Fixed Variable Names
...
try:
    ...
    for stage in ['Early_Life', 'Mid_Life', 'Late_Life', 'Near_Failure']:
        if stage in lifecycle_means.index:
            print(f"\n{stage:12s}:")
            for sensor in key_sensors:
                if sensor in lifecycle_means.columns:
...
try:
    ...
    for sensor in key_sensors:
        if sensor in variation_df.index:
...
if mae > 0:
    ...
    good_sensors = len([s for s in ['T50', 'Nf', 'Nc', 'P50'] if s in variation_df.index and variation_df.loc[s, 'Between_Engine_Ratio'] < 0.3])
    print(f"✅ Key insight: {good_sensors} sensors show strong within-engine variation")
